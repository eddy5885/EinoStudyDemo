...
func openaiHTTPClient() *http.Client {
	if os.Getenv("EINO_INSECURE_TLS") != "1" {
		// Some upstream clients may not default nil safely.
		// Use http.DefaultClient unless we explicitly need a custom Transport.
...
func defaultQuestion() string {
	return envOr("LESSON11_QUESTION", "NeoStack 公司总部在哪里？")
}
