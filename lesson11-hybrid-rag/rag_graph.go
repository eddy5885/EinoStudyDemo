...
func getHybridRAGGraph(ctx context.Context, cm model.BaseChatModel) (compose.Runnable[string, *schema.Message], error) {
	ragOnce.Do(func() {
		ragRun, ragErr = buildRAGGraph(ctx, cm, newHybridRetriever(ctx))
	})
