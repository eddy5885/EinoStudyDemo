...
var (
	...
	ragErr  error
	kbStore *memKnowledgeStore
)

func getKnowledgeStore() *memKnowledgeStore {
	if kbStore == nil {
		kbStore = newMemKnowledgeStore()
	}
	return kbStore
}

func ensureIndexed(ctx context.Context) error {
...
func getRAGGraph(ctx context.Context, cm model.BaseChatModel) (compose.Runnable[string, *schema.Message], error) {
	ragOnce.Do(func() {
		ragRun, ragErr = buildRAGGraph(ctx, cm, getKnowledgeStore())
	})
	return ragRun, ragErr
}

// buildRAGGraph：Retrieve（不调模型）→ 拼 context → Template → ChatModel。
